...
class EmployeeDirectory extends Component {
  ...
  handleStateInput = () => {
    ...
    return this.setState({
      results: data.filter(
        (employee) => employee.location.state === choice.value
      ),
    });
  };
  ...
  handleNameInput = () => {
    ...
    const data = this.state.employees;
    return this.setState({
      results: data.filter((employee) => employee.name.first === choice.value),
    });
  };

  handleLastNameInput = () => {
    const choice = document.getElementById("lastNameInput");
    const data = this.state.employees;
    return this.setState({
